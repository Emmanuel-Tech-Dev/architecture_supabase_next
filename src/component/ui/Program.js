...
const Program = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      filterMatchMode: FilterMatchMode.CONTAINS,
      body: (rowData) => (
        <span>{utils.percentageFormatter(rowData?.commission_value)}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      body: (rowData) => (
        <CustomTag
          value={rowData?.is_active ? "Yes" : "No"}
          severity={utils.getColumnStatusColorV3(rowData?.is_active)}
        />
      ),
    },
    // {
